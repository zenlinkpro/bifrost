...
use node_primitives::{
	AccountId, Balance, Block, BlockNumber, Hash, Header, Nonce, ParaId as BifrostParaId, PoolId,CurrencyId
};
	...
	if config.chain_spec.is_bifrost_kusama() {
		#[cfg(any(feature = "with-bifrost-kusama-runtime", feature = "with-bifrost-runtime"))]
		{
			let PartialComponents { client, backend, import_queue, task_manager, .. } =
				new_partial::<bifrost_kusama_runtime::RuntimeApi>(config, false)?;
...
pub trait RuntimeApiCollection:
	...
	+ stable_amm_runtime_api::StableAmmApi<Block, CurrencyId, Balance, AccountId, PoolId>
where
...
pub enum Client {
	...
	#[allow(dead_code)]
	Bifrost(Arc<FullClient<bifrost_kusama_runtime::RuntimeApi>>),
}
